...
public class GeigerCounterRender {
    ...
    public static void drawGeigerCounter(GuiGraphics context, DeltaTracker ignored) {
        if (!shouldDraw()) return;

        final int x = 5;
        final int y = context.guiHeight() - 20;
        ...
        context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, x, y, 0, 0, 94, 18, 128, 128);
        context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, x + 1, y + 1, 1, 19, (int) (radPercentage * 73), 16, 128, 128);
        ...
        if (incomingMilliRads >= THRESHOLD_1) {
            ...
            context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, x + 76, y - 18, u, 36, 18, 18, 128, 128);
        }
        if (incomingMilliRads > 0) {
            ...
            context.drawString(textRenderer, text, x + 2, y - textRenderer.lineHeight, COLOR, true);
        }
    }
}
