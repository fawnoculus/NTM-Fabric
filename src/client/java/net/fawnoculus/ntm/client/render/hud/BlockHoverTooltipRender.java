...
public class BlockHoverTooltipRender {
    ...
    public static void drawBlockHoverTooltip(GuiGraphics context, DeltaTracker ignored) {
        if (!shouldDraw()) return;
        ClientLevel world = ClientUtil.getWorld();
        ...
        Font textRenderer = ClientUtil.getTextRenderer();
        List<Component> tooltip = hoverTooltipBlock.getTooltip(world, pos, state);

        int y = (context.guiHeight() / 2) - (textRenderer.lineHeight * tooltip.size() / 2);
        final int x = context.guiWidth() / 2 + 7;

        for (Component text : tooltip) {
            context.drawString(textRenderer, text, x, y, ARGB.color(256, 256, 256), true);
